...
export default function EditPromptPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const updated = await updatePrompt(params.id, {
        title,
        description,
        content,
        category,
      ...
      router.push(`/prompt/${updated._id}`);
    } catch {
  ...
  if (isLoading) {
    return <div className="p-8 text-slate-300">Loading editor...</div>;
  }

  return (
    <div className="mx-auto min-h-screen w-full max-w-3xl p-6 text-white lg:p-10">
      <h1 className="mb-6 text-3xl font-bold">Edit Prompt</h1>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3">
          <Button type="button" variant="secondary" onClick={() => router.push(`/prompt/${params.id}`)}>Cancel</Button>
